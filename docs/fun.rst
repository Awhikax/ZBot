...
However, if too much fun bothers you, this option can be disabled at any time with the `sconfig command <sconfig.html>`_ (`enable_fun` parameter). The choice is yours!
...
You want some good news? If you have permission to use the `say <sconfig.html#list-of-every-option>`_ command, the bot will delete your message after posting it !
...
When you need the bot to talk about you, like *"Z_runner runs away very fast"*, use this command. The "me" will be replaced by your nickname, and if you are allowed to use the `say <sconfig.html#list-of-every-option>`_ command, your original message will be deleted.
