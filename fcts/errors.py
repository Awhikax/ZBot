...
class ErrorsCog:
    ...
    async def on_cmd_error(self,ctx,error):
        ...
        ignored = (commands.CommandNotFound,commands.CheckFailure,commands.ConversionError)
...
```""".format(T=" ".join(traceback.format_tb(sysexc[2])),S=s+' : '+str(sysexc[1]))
            ...
            if ctx == None:
                await salon.send("Internal Error\n"+msg)
            elif ctx.guild == None:
                await salon.send("DM | "+ctx.channel.recipient.name+"\n"+msg)
            else:
