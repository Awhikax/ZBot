...
class ModeratorCog:
    ...
    async def clear(self,ctx,number:int,*,params=''):
        ...
        if not ctx.channel.permissions_for(ctx.guild.me).manage_messages:
            await ctx.send("Permission \"Manage messages\" manquante")
            return
        if not ctx.channel.permissions_for(ctx.guild.me).read_message_history:
            await ctx.send("Permission \"Read message history\" manquante")
            return
        if number<1:
    ...
    async def kick(self,ctx,user:discord.Member,*,reason="Unspecified"):
        ...
        try:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","kick")).format(member=user,reason=reason,case=case.id)
    ...
    async def warn(self,ctx,user:discord.Member,*,message):
        ...
        try:
            if await self.bot.cogs["ServerCog"].staff_finder(user,"warn"):
        ...
        try:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","warn")).format(member=user,reason=message,case=case.id)
    ...
    async def mute(self,ctx,user:discord.Member,*,reason="Unspecified"):
        ...
        try:
            if await self.bot.cogs["ServerCog"].staff_finder(user,"mute") or user==ctx.guild.me:
        ...
        try:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","mute-on")).format(member=user,reason=reason,case=case.id)
    ...
    async def ban(self,ctx,user,*,reason="Unspecified"):
        ...
        try:
            ...
            if user in ctx.guild.members:
                ...
                if await self.bot.cogs["ServerCog"].staff_finder(member,"ban") or user==ctx.guild.me:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","ban")).format(member=user,reason=reason,case=case.id)
    ...
    async def unban(self,ctx,user,*,reason="Unspecified"):
        ...
        try:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","unban")).format(member=user,reason=reason,case=case.id)
            await self.bot.cogs["Events"].send_logs_per_server(ctx.guild,"ban",log,ctx.author)
        except Exception as e:
            await ctx.send(await self.translate(ctx.guild.id,"modo","error"))
            await self.bot.cogs['ErrorsCog'].on_error(e,ctx)
