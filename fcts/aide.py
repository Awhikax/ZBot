...
If the bot can't send the new command format, it will try to send the old one. Enable "Embed Links" permission for better rendering."""
        ...
        ft = await self.translate(ctx.guild.id,"aide","footer")
        if destination.permissions_for(me).embed_links:
            for page in pages:
                embed = self.bot.cogs["EmbedCog"].Embed(desc=page,footer_text=ft.format(await self.bot.get_prefix(ctx.message))).update_timestamp().discord_embed()
                if ctx.guild != None:
    ...
    async def all_commands(self,ctx):
        ...
        for cmd in cmds:
            ...
            if cmd.cog_name in ['AdminCog','ServerCog','ModeratorCog','CasesCog']:
                modhelp += "\n"+temp
            else:
                otherhelp += "\n"+temp
        tr = await self.translate(ctx.guild.id,"aide","mods")
    ...
    async def cog_commands(self,ctx,cog):
        ...
        if description == None:
            description = await self.translate(ctx.guild.id,"aide","no-desc-cog")
    ...
    async def cmd_help(self,ctx,cmd):
        ...
        desc = cmd.description if cmd.description!=None else str(await self.translate(ctx.guild.id,"aide","no-desc-cmd"))
        if desc=='':
            desc = cmd.help
        prefix = await self.bot.get_prefix(ctx.message)
        syntax = cmd.signature.replace(" ","** ",1) if " " in cmd.signature else cmd.signature+"**"
        if type(cmd)==commands.core.Group:
            subcmds = "\n\n__{}__".format(str(await self.translate(ctx.guild.id,"keywords","subcmds")).capitalize())
            sublist = list()
