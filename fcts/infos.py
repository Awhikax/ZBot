...
class InfosCog:
    ...
    async def stats(self,ctx):
        ...
        #r = requests.get('https://discordapp.com/api/v6').elapsed.total_seconds()
        r = self.bot.latency
        len_users = 0
        users = list()
        len_servers = 0
        for x in ctx.bot.guilds:
            if (not await self.bot.cogs['UtilitiesCog'].global_check(x)) or x.id in self.bot.cogs['ReloadsCog'].ignored_guilds:
                continue
            len_servers += 1
            for m in x.members:
                if m in users:
                ...
                len_users += 1
                users.append(m)
        d = str(await self.translate(ctx.guild,"infos","stats")).format(bot_version,len_servers,len_users,version,discord.__version__,round(py.memory_info()[0]/2.**30,3),psutil.cpu_percent(),round(r*1000,3))
        embed = discord.Embed(title="**Statistiques du bot**", colour=discord.Colour(0x63a718), timestamp=ctx.message.created_at,description=d)
...
Available types: member, role, user, emoji, channel, guild, invite, category"""
    ...
    async def member_infos(self,ctx,item,lang):
        ...
        embed.add_field(name=await self.translate(ctx.guild.id,"stats_infos","member-0"), value=item.nick if item.nick else "Aucun",inline=True)
    ...
    async def find_user(self,ctx,ID:int):
        u = await self.bot.get_user_info(ID)
        if u == None:
