...
class ModeratorCog:
    ...
    async def clear(self,ctx,number:int,*,params=''):
        ...
        if not ctx.channel.permissions_for(ctx.guild.me).manage_messages:
            await ctx.send("Permission \"Manage messages\" needed")
            return
        if not ctx.channel.permissions_for(ctx.guild.me).read_message_history:
            await ctx.send("Permission \"Read message history\" needed")
            return
        if number<1:
    ...
    async def kick(self,ctx,user:discord.Member,*,reason="Unspecified"):
        ...
        Please note that since our database is currently offline, no records can be saved."""
        try:
            if not ctx.channel.permissions_for(ctx.guild.me).kick_members:
                ...
                return
            if ctx.channel.permissions_for(user).kick_members or user==ctx.guild.me:
                await ctx.send(await self.translate(ctx.guild.id,"modo","staff-kick"))
                return
            ...
            log = str(await self.translate(ctx.guild.id,"logs","kick")).format(member=user,reason=reason,case="'unsaved'")
    ...
    async def warn(self,ctx,user:discord.Member,*,message):
        ...
        Please note that since our database is currently offline, no records can be saved."""
        try:
            if ctx.channel.permissions_for(user).manage_roles:
                await ctx.send(await self.translate(ctx.guild.id,"modo","staff-warn"))
                return
        ...
        try:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","warn")).format(member=user,reason=message,case="'unsaved'")
            await ctx.send("As our database is offline, the warn could not be backed up. Nevertheless, the member did receive his warning in DM.")
    ...
    async def mute(self,ctx,user:discord.Member,*,reason="Unspecified"):
        ...
        Please note that since our database is currently offline, no records can be saved."""
        try:
            if ctx.channel.permissions_for(user).manage_roles or user==ctx.guild.me:
        ...
        try:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","mute-on")).format(member=user,reason=reason,case="'unknown'")
    ...
    async def ban(self,ctx,user,*,reason="Unspecified"):
        ...
        try:
            ...
            if user in ctx.guild.members:
                ...
                if ctx.channel.permissions_for(member).ban_members or user==ctx.guild.me:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","ban")).format(member=user,reason=reason,case="'unsaved'")
    ...
    async def unban(self,ctx,user,*,reason="Unspecified"):
        ...
        try:
            ...
            log = str(await self.translate(ctx.guild.id,"logs","unban")).format(member=user,reason=reason,case="'unsaved'")
            await self.bot.cogs["Events"].send_logs_per_server(ctx.guild,"ban",log,ctx.author)
        except Exception as e:
            await ctx.send(await self.translate(ctx.guild.id,"modo","error"))
            await self.bot.cogs['ErrorsCog'].on_error(e,ctx)
