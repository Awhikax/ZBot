...
async def is_fun_enabled(ctx):
    return ctx.author.id in reloads.admins_id or ctx.channel.permissions_for(ctx.author).manage_guild
...
class FunCog:
    ...
    async def main(self,ctx):
        ...
        if not await is_fun_enabled(ctx):
            await ctx.send("As our database is offline, access to fun commands has been restricted to people with \"Manage Server\" permission.")
            return
...
You can specify a verification limit by adding a number in argument"""
    ...
    async def say(self,ctx,channel:typing.Optional[discord.TextChannel] = None,*,text):
        ...
        This command can only be used by administrators while our database stays offline"""
